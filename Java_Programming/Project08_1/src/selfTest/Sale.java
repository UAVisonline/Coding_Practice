...
public class Sale {
	...
	public boolean equalDeals(Sale otherSale) {
		...
		else if(otherSale.bill()==this.bill() && this.getName().equals(otherSale.getName())) return true;
		else return false;
	}
	
	public boolean lessThan(Sale otherSale) {
		if(otherSale==null) {
			System.out.println("Fatal Error : Null Exception");
			System.exit(0);
		}
		if(otherSale.bill()>this.bill()) return true;
		else return false;
	}
