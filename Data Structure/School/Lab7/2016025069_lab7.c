...
void Insert(BTTree* Tree, DATATYPE Data, DATATYPE Parent)
{
	AddressNode* temp = (AddressNode*)malloc(sizeof(AddressNode)); // Init AddressNode
	temp->Data = Data;
	temp->Parent = Parent;
	temp->next = NULL;

	if (!PosHead(Tree, &Data)) Tree->Head->next = temp; // First Insert node
	else
	{
		while (1)
		{
			if (!PosNext(Tree, &Data)) // Find empty next null
			{
				Tree->Curr->next = temp;// Insert Node
				break;
			}
		}
	}
	Tree->NumofData++; //Num data size up
	/*
	if (Tree->Head->next == NULL) //Linked Likt is empty
	{
		Tree->Head->next = temp; // Insert node
		Tree->Curr = Tree->Head->next; // Current node change
		//temp = Tree->Curr;
	}
	else
	{
		Tree->Curr->next = temp; // Insert node
		Tree->Curr = Tree->Curr->next; // Current Node change
	}
	*/
}
...
DATATYPE Search(BTTree* Tree, DATATYPE* Data)
{
	int find_data = Data; //data store int variable
	if (PosHead(Tree, &Data)) // Find data
	{
		if (find_data == Data) return Tree->Curr->Parent; //if data equals Data return Parent
		while (PosNext(Tree, &Data)) //Find all of the node equal data
		{
			if (find_data == Data) return Tree->Curr->Parent; //if data equals Data return Parent
		}
	}
...
int RetTotalHeight(BTTree* Tree)
{
	if (RetHeight(Tree->Root) == 1) return 0; // if Tree has only root node-> return 0
	else if (RetHeight(Tree->Root) > 1) //else Tree has child node
	{
		int height = RetHeight(Tree->Root)-1; // return height of tree-1
		return height;
	}
}
...
DATATYPE MakeAddressMapWithSubTree(BTTree* Tree, BTNode* Node)
{
	if (Node == NULL)//if child node is empty
	{
		return NULL;
	}
	else
	{
		int Data = MakeAddressMapWithSubTree(Tree, RetSubTreeLeft(Node)); // Left child node
		if (Data != NULL)
		{
			Insert(Tree, Data, Node->Data); // Insert 
		}

		Data = MakeAddressMapWithSubTree(Tree, RetSubTreeRight(Node)); // Right child node
		if (Data != NULL)
		{
			Insert(Tree, Data, Node->Data); // Insert
		}
	}
	return Node->Data; // return this data for child node returned value
}

DATATYPE Find(BTTree* Tree, DATATYPE* Data)
{
	int parent = Search(Tree, Data); // Find parent value
	if (parent > 0) return Find(Tree, parent); // if the node is not root
	else if (parent <= 0) return Tree->Curr->Data; // it the node is root, return the node of data
}

int Union(BTTree* Tree1, BTTree* Tree2)
{
	int Height_1 = Search(Tree1, Tree1->Root->Data);
	int Height_2 = Search(Tree2, Tree2->Root->Data);
	int* Data = (int*)malloc(sizeof(int)); //malloc int pointer to use function
	if (Height_1 > Height_2) // Tree 1 go to Tree 2 
	{
		PosHead(Tree1, Data); // Find first data
		InsertBST(&Tree2->Root,*Data); // Insert First node
		while (PosNext(Tree1, Data)) //Find others node
		{
			InsertBST(&Tree2->Root, *Data); // Insert others node
		}
		MakeAddressMap(Tree2); // remake address map
		return 2;
	}
	else // Tree 2 go to Tree 1
	{
		PosHead(Tree2, Data);
